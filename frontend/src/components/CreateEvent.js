...
function doRequest(endpoint, id, data, callback, method = "POST") {
  ...
  xhr.open(
    method,
    "https://DEPLOY_WEBSITE_ADDRESS/" + endpoint + "/" + id,
    true
  );
...
function changeEventState(endpoint, id, params, callback, method = "POST") {
  ...
  xhr.open(
    method,
    "https://DEPLOY_WEBSITE_ADDRESS/" + endpoint + "/" + id + "?" + params,
    true
  );
  console.log(
    "https://DEPLOY_WEBSITE_ADDRESS/" + endpoint + "/" + id + "?" + params
...
function deletePlayer(element) {
  doRequest(
    "event-manager/remove-player",
    document.getElementById("Event_id").value +
      "/" +
      element.target.parentNode.children[0].value,
    "",
    () => {
      ...
      <div>
        ...
        <div className="row">
          <button
            ...
            onClick={() => {
              changeEventState(
                "event-manager/change-event-state",
                document.getElementById("Event_id").value,
                ...
                onClick={() => {
                  doRequest(
                    ...
                    (json) => {
                      ...
                      document.getElementById("EventCreate").textContent =
                        "Created";
