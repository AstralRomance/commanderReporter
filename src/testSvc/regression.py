...
class TestBaseEventWorkflow:
    ...
    @pytest.mark.parametrize('rounds_number', [random.randint(3, 5)])
            ...
            for event_round in event_info['Rounds']:
                for table in event_round['Players_on_table']:
                    ...
                    update_points_response = requests.put(update_points_endpoint+table_winner['id'],
                                                          params={'round_num': round_number}, data={'Points': 3, 'Sub_points': random.randint(1, 4)})
                    assert update_points_response.ok, update_points_response
